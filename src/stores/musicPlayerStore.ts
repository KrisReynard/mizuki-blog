...
class MusicPlayerStore {
    ...
    private createInitialState(): MusicPlayerState {
        let defaultRepeating: RepeatMode = 0; // 0: 列表循环, 1: 单曲循环, 2: 随机播放
        if (musicPlayerConfig.defaultPlayMode === "repeat-one") {
            defaultRepeating = 1;
        } else if (musicPlayerConfig.defaultPlayMode === "random") {
            defaultRepeating = 2;
        }
        ...
        return {
            ...
            volume: 0.7,
            isMuted: false,
            isShuffled: musicPlayerConfig.defaultPlayMode === "random",
            isRepeating: defaultRepeating === 2 ? 0 : defaultRepeating,
            showPlaylist: false,
            errorMessage: "",
            showError: false,
            isExpanded: false,
